...
else if(!preg_match('/^[0-9]{10}+$/', $phone)&&!empty($_POST["email"])){
...
if($i==2)
{
	...
	$sql1="SELECT * FROM st_reg WHERE email='$email'";
	
	$qry1=mysql_query($sql1) or die(mysql_error());
	
	$rw1=mysql_num_rows($qry1);
    ...
    $pass=md5($v3);
    $email=md5($v4);
	//$sql="INSERT INTO st_reg(name,dob,email,phone,college,admno,yr_study) Values('re','12/2/1990','raj@gmail.com','8893335148','lmcst','100','2011-2016)";
	$sql="INSERT INTO st_reg(name,dob,email,phone,college,admno,yr_study) Values('$v1','$v2','$v3','$v4','$v5','$v6','$v7')";
	mysql_query($sql) OR die(mysql_error());
	 	
		$maxval= mysql_query("SELECT MAX(id) FROM st_reg");
	$val=mysql_fetch_array($maxval);
	$max= $val[0];
	mysql_query("INSERT INTO login(st_id,email,password,actype) Values('$max','$pass','$email','student')");
...
{
	echo"<script> alert('invalid email and phone number'); </script>";
	}
}
...
?>
                    ...
                    <div class="form">
                       ...
                       <div class="form-group">
                            ...
                            <option value="LMCST">LMCST</option>
                         	<option value="CET">CET</option>
                         	<option value="MCET">MCET</option>
                         	<option value="TKM">TKM</option>
                         	<option value="SCMS">SCMS</option>
                         	<option value="VIDYA ACADEMY">VIDYA ACADEMY</option>
							<option value="ST.JOSEPHS">ST.JOSEPHS COLLEGE PALAI</option>
							<option value="FISAT">FISAT</option>
							<option value="NEHRU COLLEGE">NEHRU COLLEGE</option>
							<option value="MAR ATHANASIUS">MAR ATHANASIUS</option>
							<option value="SAINTGITS COLLEGE">SAINTGITS COLLEGE</option>
							<option value="RIT">RIT</option>
							<option value="AMAL JYOTHI">AMAL JYOTHI</option>
							<option value="GEC THRISSUR">GEC THRISSUR</option>
							<option value="SREE NARAYANA GURUKULAM">SREE NARAYANA GURUKULAM</option>
							<option value="COLLEGE OF ENGINEERING VADAKARA">COLLEGE OF ENGINEERING VADAKARA</option>
							<option value="AWH ENGINEERING COLLEGE">AWH ENGINEERING COLLEGE</option>
							<option value="MES KUTTIPPURAM">MES KUTTIPPURAM</option>
                         	</select>
                         </div>
                        
                        
                          <div class="input-container">
                         	<input type="text" name="admno" placeholder="Addmission number">
                         </div>
                          <div class="input-container">
                        ...
                        <div class="input-container">
                            <button type="submit" class="btn-style" name="submit">Register</button>
                        </div>
                    </div>
						</form>
...
if($rc!=0){
	...
	$id=$row['st_id']; echo $id;
	
	if($actype=="staff"){
		
		$_SESSION['uname'] = $mail;
		$_SESSION['uid'] = $id;
		echo "<script>window.location.replace('staff/index.html');</script>";
	
	}elseif($actype=="student"){
		$sql = "select * from st_reg where email = '" . $s1 . "'";
		$q=mysql_query($sql) or die(mysql_error());
		$id = mysql_fetch_array($q)[0];
		$_SESSION['uname']=$mail;
		$_SESSION['uid']=$id;
	
		echo "<script>window.location.replace('students/student-prof.php');</script>";
	...
	<div id="sidr">
    					...
    					<div class="col-md-10">
    						...
    						<div class="nav_2" id="navigation">
    							<ul>
		                            ...
		                            <li><a href="#">Books</a>
		                            	<ul>
		                                	<li><a href="our-courses.php">Books</a></li>
		...
		<div class="edu2_main_bn_wrap">
			<div id="owl-demo-main" class="owl-carousel owl-theme">
				<div class="item">
					<figure>
						<img src="extra-images/ktuu2.jpg" alt=""/>
						<figcaption>
							<span><h2>We Invites You...</h2></span>
				...
				<div class="item">
					<figure>
						<img src="extra-images/ktuu1.jpg" alt=""/>
						<figcaption>
